# This script evaluates classification performance using LDA 
...
for(r in 1:runs){
  ...
  p1<-predict(lda(tr1,grouping=ytr,tol=1e-4),te1)$class
  p2<-predict(lda(tr2,grouping=ytr,tol=1e-4),te2)$class
  p3<-predict(lda(tr3,grouping=ytr,tol=1e-4),te3)$class
  p4<-predict(lda(tr4,grouping=ytr,tol=1e-4),te4)$class
